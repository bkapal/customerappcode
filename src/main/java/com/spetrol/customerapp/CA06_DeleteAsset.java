...
public class CA06_DeleteAsset extends TestBase {
	...
	public void testLogout() throws InterruptedException, IOException {
		...
		Thread.sleep(2000);
		wait.until(buildExpectedConditions(Constants.Lnk_EditAsset)).click();
		wait.until(buildExpectedConditions(Constants.Lnk_DeleteAsset)).click();
		wait.until(buildExpectedConditions(Constants.Btn_DeleteConfirm)).click();
		Thread.sleep(2000);
	}
}
