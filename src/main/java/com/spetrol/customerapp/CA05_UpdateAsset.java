...
public class CA05_UpdateAsset extends TestBase {
	...
	public void testLogout() throws InterruptedException, IOException {
		...
		Thread.sleep(2000);
		wait.until(buildExpectedConditions(Constants.Lnk_UpdateAssetIndusMach)).click();
		wait.until(buildExpectedConditions(Constants.TxtBox_UpdatedFuel)).sendKeys("300");
		wait.until(buildExpectedConditions(Constants.Btn_UpdateAsset)).click();
		Thread.sleep(2000);
	}
}
